...
export default function App() {
  ...
  return <main className={`app-shell ${mode}`}><ModeSwitch mode={mode} setMode={setMode} />{mode === "curator" && <p className="top-tools"><a href="/tools">馆长的工具箱</a></p>}<header><h1>复古游戏博物馆</h1><input placeholder="搜索游戏名" value={query} onChange={(event) => setQuery(event.target.value)} /></header>{mode === "curator" && <><p><a href="/new">新建游戏</a></p><QuickCreateGame onDone={refreshGames} /></>}{error && <p className="error">{error}</p>}{!error && filtered.length === 0 && <p className="muted">{query ? "没有找到游戏" : "没有游戏"}</p>}<section className="game-list">{filtered.map((game) => <a className="game-card" href={`/game/${encodeURIComponent(game.path)}`} key={game.path}><strong>{game.name}</strong><span>{game.platform}</span></a>)}</section></main>;
}
